...
Usage:
    ...
    contextlint compare <a> <b> [options]
    contextlint rules
    contextlint --version
"""
...
from contextlint.reports.badge import badge_markdown
from contextlint.reports.json_report import report_to_dict
from contextlint.rules import BUILTIN_RULE_IDS, RULES
from contextlint.utils.ansi import Style, supports_color

_EPILOG = """\
examples:
  contextlint analyze ./docs
  contextlint analyze ./docs --format html --output report.html
  contextlint analyze export.json --format sarif -o results.sarif --fail-on error
  contextlint analyze ./docs --ignore CTX006 --select CTX001,CTX003
  contextlint compare recursive.json semantic.json
  contextlint rules
...
def _cmd_compare(args: argparse.Namespace) -> int:
    for label, p in (("before", args.before), ("after", args.after)):
        if not Path(p).exists():
            print(f"contextlint: error: {label} path not found: {p}", file=sys.stderr)
            return 2
