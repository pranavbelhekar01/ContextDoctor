...
3. **A published package** — expose an entry point so it's picked up automatically::
       ...
       my-rules = "contextlint_plugin_myrules:MyAnalyzer"

A plugin is just an :class:`~contextlint.analyzers.base.Analyzer` subclass (or a
...
from contextlint.analyzers.base import Analyzer
from contextlint.rules import register_rule

ENTRY_POINT_GROUP = "contextlint.analyzers"
...
def _load_from_file(path: Path) -> object:
    spec = importlib.util.spec_from_file_location(f"contextlint_plugin_{path.stem}", path)
...
def load_module_analyzers(specs: list[str]) -> list[type[Analyzer]]:
    ...
    for spec in specs:
        try:
            ...
            warnings.warn(f"ContextLint: failed to load plugin '{spec}': {exc}", stacklevel=2)
            continue
        classes = _coerce_analyzers(obj)
        if not classes:
            warnings.warn(f"ContextLint: no Analyzer found in plugin '{spec}'", stacklevel=2)
        for cls in classes:
            _register_rules(cls)
            found.append(cls)
    return found
...
def discover_entry_point_analyzers() -> list[type[Analyzer]]:
    """Discover analyzers published under the ``contextlint.analyzers`` group."""
    found: list[type[Analyzer]] = []
    try:
    ...
    for ep in entry_points:
        try:
            ...
            warnings.warn(f"ContextLint: failed to load plugin '{ep.name}': {exc}", stacklevel=2)
            continue
