...
Supported anywhere in a file's text (e.g. in a Markdown comment):
    ...
    <!-- contextlint: disable=CTX007,CTX008 --> # comma-separated
    <!-- contextlint: disable-all -->           # disable every rule for this file
    # contextlint: disable=CTX003               # the marker works in any comment style

Pragmas are **file-scoped** (they apply to the whole document), which maps
cleanly onto ContextLint's aggregate, per-rule findings. ``"*"`` in the returned
set means "all rules".
"""
...
_PRAGMA_RE = re.compile(
    r"contextlint:\s*disable(?P<all>-all)?(?:\s*=\s*(?P<ids>[A-Za-z0-9_,\s]+))?",
    re.IGNORECASE,
)
...
def parse_disabled_rules(text: str) -> set[str]:
    ...
    for m in _PRAGMA_RE.finditer(text):
        ...
        if ids:
            ...
            # A bare "contextlint: disable" with no ids disables everything.
            disabled.add("*")
    return disabled
