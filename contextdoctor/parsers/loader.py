...
For ``.md`` / ``.txt`` inputs ContextDoctor chunks the raw text itself. For
...
from contextdoctor.parsers.formats import (
    csv_rows_to_chunks,
    html_to_text,
    jsonl_to_texts,
    pdf_to_text,
...
def _is_hidden(file: Path, root: Path) -> bool:
    """Skip dotfiles and dot-directories (e.g. .contextdoctor.json, .git/)."""
    try:
