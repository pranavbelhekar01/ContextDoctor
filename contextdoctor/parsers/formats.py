...
ContextDoctor's zero-dependency core. PDF is *optional*: it uses ``pypdf`` if
installed (``pip install contextdoctor[pdf]``) and otherwise raises a clear error.
"""
...
def jsonl_to_texts(raw: str) -> list[str]:
    ...
    from contextdoctor.parsers.loader import extract_chunk_text
...
def pdf_to_text(path: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            'Install it with: pip install "contextdoctor[pdf]"'
        ) from exc
