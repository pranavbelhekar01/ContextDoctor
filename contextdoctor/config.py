...
loaded from a JSON file (``--config``) or a ``[tool.contextdoctor]`` table in
...
@dataclass
class Config:
    """Tunable thresholds for ContextDoctor's analyzers and chunker."""
    ...
    def load(cls, path: str | Path) -> Config:
        """Load config from a JSON file, or a pyproject.toml with [tool.contextdoctor]."""
        path = Path(path)
        ...
        if path.suffix == ".toml" or path.name == "pyproject.toml":
            ...
            data = data.get("tool", {}).get("contextdoctor", {})
        else:
    ...
    def discover(cls, start: str | Path) -> Config:
        ...
        Search order (first hit wins): ``.contextdoctor.json`` then
        ``pyproject.toml`` (with a ``[tool.contextdoctor]`` table), walking up
        from ``start`` to the filesystem root.
        """
        start = Path(start).resolve()
        base = start if start.is_dir() else start.parent
        for directory in [base, *base.parents]:
            candidate = directory / ".contextdoctor.json"
            ...
            if pyproject.is_file():
                try:
                    ...
                    data = {}
                if data.get("tool", {}).get("contextdoctor"):
                    return cls.from_dict(data["tool"]["contextdoctor"])
        return cls()
