# ContextDoctor examples

Runnable datasets that demonstrate what ContextDoctor catches. All analysis is
fully offline.
...
| [`fragmented_kb/`](fragmented_kb) | A knowledge base where key systems are introduced at the top and only revisited at the very bottom, maximising fragmentation. Ships its own `.contextdoctor.json`. | CTX006 (experimental CFI) |
| [`mixed_formats/`](mixed_formats) | The same corpus as HTML, CSV, and JSONL — shows the multi-format parsers. | CTX002, CTX003 |
| [`pragma_demo/`](pragma_demo) | A doc that *shows* an example API key and opts out with an inline `contextdoctor: disable=CTX007` pragma. | _none_ (CTX007 suppressed) |
...
contextdoctor analyze ./examples/clean_docs
